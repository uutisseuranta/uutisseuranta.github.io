...
| 2026-08-18 | Tagin lisäyksen in-place DOM-päivitys (L-026) | Päivitetään uusi tagi onnistuneen tallennuksen jälkeen suoraan kyseisen kortin DOM-rakenteeseen ilman `refreshFeed()`-kutsua. Estää uutiskortin ja vierityskohdan katoamisen ja mahdollistaa artikkelin kommentoinnin tai lukemisen jatkamisen heti tagilisäyksen jälkeen. | Koko uutisvirran uudelleenlataus tagin lisäyksen jälkeen (aiheutti artikkelin häviämisen) | Jos siirrytään reaaliaikaiseen reaktiovirtaan | DECISION_LOG.csv;USER_PATHS.md |
| 2026-08-18 | Progressiivinen uutisvirran laajennus ja luettujen eräkuittaus (L-025) | Haetaan 500 uutisen lukematon perusjoukko kerralla muistiin ja laajennetaan näkymää progressiivisesti (1–5 -> 1–50 -> 1–500) säilyttäen samat alkuosan kortit paikoillaan; luettujen tunnisteet kuitataan palvelimelle vasta setin päätteeksi. | Erilliset välihaut ja luettujen ennenaikainen poissulkeminen kesken laajennuksen | Jos siirrytään palvelinpuolen streamaukseen | DECISION_LOG.csv;USER_PATHS.md |
| 2026-08-18 | Uutisvirran saumaton append-lisäyssivutus (L-024) | Muutettiin `renderFeed` tukemaan append-tilaa (`append = true`) ja kasvatetaan uutiskokoelmaa kumulatiivisesti ilman uutisruudukon tyhjentämistä (`grid.innerHTML = ''`), jolloin luetut kortit säilyvät DOMissa ja vierityskohta pysyy vakaana. | Koko uutisruudukon tyhjennys ja uudelleenrenderöinti sivutettaessa (aiheutti räpsymistä ja hyppimistä) | Jos siirrytään virtuaalilistaukseen (virtualized list) | DECISION_LOG.csv;USER_PATHS.md |
| 2026-08-18 | Python-pohjaiset LLM-skillit repo-hygienialle ja validoinnille (L-023) | Toteutettiin repo-hygienian, päätöslokin validoinnin, build- ja smoke-testauksen säännöt paikallisiksi Python-skilleiksi (`.agents/skills/`). Säästää LLM-päättelytokeneita ja nopeuttaa tarkistuksia. | Pelkät etäajettavat GitHub Actions -työnkulut | Jos skill-rajapinta korvataan toisella agenttijärjestelmällä | DECISION_LOG.csv |
